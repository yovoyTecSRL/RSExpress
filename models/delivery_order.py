...
class RSExpressDeliveryOrder(models.Model):
    ...
    package_description = fields.Text(
        string="Descripción del Paquete"
    )
    ...
    incident_description = fields.Text(
        string="Descripción de Incidencia"
    )
    ...
    def _compute_name(self):
        for record in self:
            if record.order_code and record.order_code != _('Nuevo'):
    ...
    _sql_constraints = [
        ('order_code_unique', 'UNIQUE(order_code)',
         'El código de orden debe ser único!')
    ]
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('order_code', _('Nuevo')) == _('Nuevo'):
                vals['order_code'] = self.env['ir.sequence'].next_by_code('rsexpress.delivery.order') or _('Nuevo')
        return super().create(vals_list)

    # ========== MÉTODOS DE CAMBIO DE ESTADO ==========

    def action_assign(self):
        """Asigna la orden a un vehículo y conductor"""
        for record in self:
            if not record.vehicle_id:
                raise UserError(_("Debe asignar un vehículo antes de continuar"))
            
            if record.state != 'new':
                raise UserError(_("Solo se pueden asignar órdenes nuevas"))
            
            record.state = 'assigned'
            record.message_post(
                body=_("📋 Orden asignada a vehículo %s") % record.vehicle_id.license_plate,
    ...
    def action_pickup(self):
        ...
        for record in self:
            ...
            record.message_post(
                body=_("🚗 Mensajero en ruta hacia punto de recogida"),
                subject=_("En Ruta")
            )
            ...
            # Notificar cliente
            record.send_whatsapp_notification(
                f"El mensajero está en camino a recoger tu paquete. Orden: {record.order_code}"
            )
    ...
    def action_package(self):
        ...
        for record in self:
            ...
            # Notificar cliente
            record.send_whatsapp_notification(
                f"Tu paquete ha sido recogido y está en camino. Orden: {record.order_code}"
            )
    ...
    def action_delivering(self):
        ...
        for record in self:
            ...
            record.message_post(
                body=_("🚚 En camino de entrega al cliente"),
            ...
            # Notificar cliente
            record.send_whatsapp_notification(
                f"Tu paquete está en camino. Llegaremos pronto. Orden: {record.order_code}"
            )
    ...
    def action_delivered(self):
        ...
        for record in self:
            ...
            record.message_post(
                body=_("✅ Orden entregada exitosamente"),
            ...
            # Notificar cliente
            record.send_whatsapp_notification(
                f"✅ Tu paquete ha sido entregado. Gracias por usar RSExpress. Orden: {record.order_code}"
            )
    ...
    def action_incident(self):
        ...
        for record in self:
            if record.state in ['delivered', 'cancelled']:
                raise UserError(_("No se puede marcar incidencia en órdenes finalizadas"))
            
            record.state = 'incident'
            record.message_post(
                body=_("⚠️ Incidencia reportada: %s") % (record.incident_description or 'Sin detalles'),
            ...
            record.send_whatsapp_notification(
                ...
                f"Nuestro equipo está trabajando en solucionarlo."
            )
    ...
    def action_failed(self):
        ...
        for record in self:
            ...
            record.end_time = fields.Datetime.now()
            record.message_post(
                body=_("❌ Intento de entrega fallido"),
    ...
    def action_cancel(self):
        ...
        for record in self:
            ...
            record.message_post(
                body=_("🚫 Orden cancelada"),
                subject=_("Orden Cancelada")
            )
            
            # Actualizar vehículo si estaba asignado
            if record.vehicle_id and record.vehicle_id.x_operational_status != 'available':
    ...
    def send_whatsapp_notification(self, message):
        """
        Envía notificación WhatsApp vía Respond.io
        TODO: Implementar integración con API Respond.io
        """
        self.ensure_one()
        
        # Por ahora solo registra en chatter
        self.message_post(
            body=_("📱 Notificación WhatsApp: %s") % message,
    ...
    def update_gps_from_traccar(self, data):
        ...
        if self.vehicle_id and 'latitude' in data and 'longitude' in data:
            ...
            self.message_post(
                body=_("📍 GPS actualizado: Lat %.6f, Lon %.6f") % (data['latitude'], data['longitude']),
    ...
    def action_resend_notification(self):
        ...
        for record in self:
            ...
            message = state_messages.get(record.state, 'Actualización de tu orden')
        ...
        return {
            ...
            'params': {
                ...
                'message': _('Se ha reenviado la notificación al cliente'),
                'type': 'success',
                'sticky': False,
            }
        }